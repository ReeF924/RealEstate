...
namespace RealEstate.Controllers
{
	public class AdminController : BaseEstateController
	{
		...
		[Authorize(false)]
		public IActionResult EditOffer(int idOffer, string controller, string action)
		{
			User user = this.ViewBag.User;
			var offer = this._context.Offers!.Find(idOffer);

			if (offer!.IdBroker != user.Id || user.Type != 'a')
				return RedirectToAction(action, controller);

			var users = this._context.Users.Where(user => user.Type != 'u').ToList();
			users.Remove(user);
